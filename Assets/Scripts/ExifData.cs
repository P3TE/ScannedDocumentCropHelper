// -------------------------------------------------------------------------------------------------
//
// Library "C" for reading and writing EXIF data in JPEG, TIFF and PNG image files.
//
...
namespace CompactExifLib
{
	public class ExifData
	{
		...
		public void Save(string DestFileNameWithPath = null, ExifSaveOptions SaveOptions = 0)
		{
			...
			try
			{
				...
				{
					File.Delete(
						SourceFileNameWithPath); // Delete original image file. If the original file cannot be deleted, delete
				...
				if (GetTagItem(TagSpec, out t))
				{
					...
					if (CodePage != 0)
					{
						if (((StringTagFormat != StrCodingFormat.TypeAscii) || (t.TagType != ExifTagType.Ascii)) &&
							((StringTagFormat != StrCodingFormat.TypeUndefined) ||
						     (t.TagType != ExifTagType.Undefined)) &&
		...
		public bool GetTagValue(ExifTag TagSpec, out DateTime Value, ExifDateFormat Format = ExifDateFormat.DateAndTime)
		{
			...
			if (GetTagItem(TagSpec, out t) && (t.TagType == ExifTagType.Ascii))
			{
				try
				{
					...
					{
						Year = CalculateTwoDigitDecNumber(t.ValueData, i) * 100 +
						       CalculateTwoDigitDecNumber(t.ValueData, i + 2);
						Month = CalculateTwoDigitDecNumber(t.ValueData, i + 5);
						Day = CalculateTwoDigitDecNumber(t.ValueData, i + 8);
						if ((Format == ExifDateFormat.DateAndTime) && (t.ValueCount == 19 + 1) &&
						    (t.ValueData[i + 10] == (byte)' ') &&
						    (t.ValueData[i + 13] == (byte)':') && (t.ValueData[i + 16] == (byte)':') &&
						    (t.ValueData[i + 19] == 0))
						{
						...
						else if ((Format == ExifDateFormat.DateOnly) && (t.ValueCount == 10 + 1) &&
						         (t.ValueData[i + 10] == 0))
						{
			...
			int RawDataIndex = 0)
		{
			...
			if ((uint)Ifd < ExifIfdCount)
			{
				...
				{
					t = new TagItem(ExtractTagId(TagSpec), TagType, ValueCount, RawData, RawDataIndex,
						RawDataByteCount);
		...
		public void ReplaceAllTagsBy(ExifData SourceExifData)
		{
			...
			if (this.ImageType == ImageType.Tiff)
			{
				...
				this._ByteOrder =
					SourceExifData
						._ByteOrder; // Complete EXIF block is copied, therefore the byte order is also copied.
			...
			bool IsTiffImagePresent =
				(this.ImageType == ImageType.Tiff) || (SourceExifData.ImageType == ImageType.Tiff);
			bool CopyTag = true;
			...
			if (this.ImageType == ImageType.Jpeg)
			{
				...
				if (SourceExifData.ThumbnailImageExists())
				{
					...
					Array.Copy(SourceExifData.ThumbnailImage, SourceExifData.ThumbnailStartIndex, this.ThumbnailImage,
						0, this.ThumbnailByteCount);
				}
				else
				{
		...
		{
			return (GetDateAndTimeWithMillisecHelper(out Value, ExifTag.DateTimeDigitized,
		...
		private const int
			MinExifBlockLen =
				2 + 4; // Minimum number of bytes for an EXIF block: Value for the number of tags of IFD Primary Data (2 bytes) +
		...
		private byte[]
			SourceExifBlock; // Temporary storage for the EXIF block in JPEG and PNG images, starting at index 8.
		...
		private static readonly byte[] IdCodeUtf16 = new byte[]
			{ (byte)'U', (byte)'N', (byte)'I', (byte)'C', (byte)'O', (byte)'D', (byte)'E', 0 };

		private static readonly byte[] IdCodeAscii = new byte[]
			{ (byte)'A', (byte)'S', (byte)'C', (byte)'I', (byte)'I', 0, 0, 0 };
		...
		private int
			ThumbnailStartIndex,
			ThumbnailByteCount; // Range, in which the thumbnail image in the array "ThumbnailData" is stored
		...
		private const int
			JpegMaxExifBlockLen =
				65534 - 2 - 6 - TiffHeaderLen; // Maximum number of bytes an EXIF block in a JPEG file may have
		...
		private static readonly byte[] JpegXmpInitialSignature =
		{
		...
		private static readonly byte[] JpegIptcSignature =
		{
		...
		private const int
			TiffHeaderLen =
				8; // Size of TIFF header. The TIFF header is also present in EXIF blocks of JPEG and PNG files.
		...
		{
			(byte)'X', (byte)'M', (byte)'L', (byte)':', (byte)'c',
		...
		{
			(byte)'R', (byte)'a', (byte)'w', (byte)' ', (byte)'p',
		...
		private void ReadJepg()
		{
			...
			do
			{
				ReadJpegBlock(SourceExifStream, BlockContent, out int BlockContentSize, out BlockMarker, out BlockType,
					out MetaDataIndex);
		...
		private void ReadPng()
		{
			...
			do
			{
				...
				int BytesRead = DetectPngImageBlock(SourceExifStream, BlockContent, ChunkType,
		...
		private int DetectPngImageBlock(Stream ImageStream, byte[] SignatureData, uint ChunkType,
			out ImageFileBlock BlockType)
		{
			int BytesRead = 0;
		...
		private static readonly uint[] Crc32ChecksumTable =
		{
			...
			0xB8BDA50F,
			0x2802B89E, 0x5F058808, 0xC60CD9B2, 0xB10BE924, 0x2F6F7C87, 0x58684C11, 0xC1611DAB, 0xB6662D3D, 0x76DC4190,
			0x01DB7106,
			0x98D220BC, 0xEFD5102A, 0x71B18589, 0x06B6B51F, 0x9FBFE4A5, 0xE8B8D433, 0x7807C9A2, 0x0F00F934, 0x9609A88E,
			0xE10E9818,
			0x7F6A0DBB, 0x086D3D2D, 0x91646C97, 0xE6635C01, 0x6B6B51F4, 0x1C6C6162, 0x856530D8, 0xF262004E, 0x6C0695ED,
			0x1B01A57B,
			...
			0xA6BC5767,
			0x3FB506DD, 0x48B2364B, 0xD80D2BDA, 0xAF0A1B4C, 0x36034AF6, 0x41047A60, 0xDF60EFC3, 0xA867DF55, 0x316E8EEF,
			0x4669BE79,
		...
		private static uint CalculateCrc32(byte[] Data, int StartIndex, int Length, bool Finalize = true,
			uint StartCrc = 0xFFFFFFFF)
		{
		...
		private void UpdateImageFileBlockInfo()
		{
			...
			{
				ExifIfd[] TempIfds = new ExifIfd[]
					{ ExifIfd.PrivateData, ExifIfd.GpsInfoData, ExifIfd.Interoperability, ExifIfd.ThumbnailData };
				foreach (ExifIfd Ifd in TempIfds)
				{
		...
		private void ReadJpegBlock(Stream ImageStream, byte[] BlockContent, out int BlockContentSize,
		...
		private void SaveJpeg(Stream SourceStream, Stream DestStream)
		{
			...
			do
			{
				ReadJpegBlock(SourceStream, BlockContent, out int BlockContentSize, out BlockMarker,
					out ImageFileBlock BlockType, out _);
				if (BlockMarker == JpegSosMarker)
				{
				...
				else if ((BlockType == ImageFileBlock.Xmp) &&
				         (ImageFileBlockInfo[(int)BlockType] == ImageFileBlockState.Removed))
				{
				...
				else if ((BlockType == ImageFileBlock.JpegComment) &&
				         (ImageFileBlockInfo[(int)BlockType] == ImageFileBlockState.Removed))
				{
					CopyBlockFromSourceStream = false;
				}
		...
		private void SaveTiff(Stream SourceStream, Stream DestStream)
		{
			...
			do
			{
				CurrentImageExif = CreateSubExifData(SourceStream, ref SourceStreamExifBlockOffset,
				...
				else
				{
					...
					CurrentImageExif.ExifWriteUInt32(ExifBlockAsBinaryData.Buffer, NextExifBlockPointerIndex,
						CurrentDestStreamOffset);
				...
				NextExifBlockPointerIndex =
					CurrentImageExif.CreateExifBlock(out ExifBlockAsBinaryData, (int)CurrentDestStreamOffset);
		...
		private static ExifData CreateSubExifData(Stream TiffStream, ref int ExifBlockOffset, ExifByteOrder ByteOrder,
			out uint[] SegmentOffsetTable,
			out uint[] SegmentByteCountTable, out ExifTag SegmentOffsetsTag, out ExifTag SegmentByteCountsTag)
		{
			...
			for (int i = 0; i < ValueCount; i++)
			{
				...
				{
					v = SubExifBlock.ExifReadUInt16(SegmentOffsetsTagItem.ValueData,
						SegmentOffsetsTagItem.ValueIndex + (i << 1));
				}
				else
					v = SubExifBlock.ExifReadUInt32(SegmentOffsetsTagItem.ValueData,
						SegmentOffsetsTagItem.ValueIndex + (i << 2));

				SegmentOffsetTable[i] = v;
				if (ByteCountTable16Bit)
				{
					v = SubExifBlock.ExifReadUInt16(SegmentByteCountsTagItem.ValueData,
						SegmentByteCountsTagItem.ValueIndex + (i << 1));
				}
				else
					v = SubExifBlock.ExifReadUInt32(SegmentByteCountsTagItem.ValueData,
						SegmentByteCountsTagItem.ValueIndex + (i << 2));
		...
		private void WriteTagToFlexArray(TagItem TempTagData, FlexArray WriteData, ref int TagDataIndex,
			int ExifBlockOffset)
		{
			int i, ByteCount;
			byte v;
			...
			else
			{
				...
				Array.Copy(TempTagData.ValueData, TempTagData.ValueIndex, WriteData.Buffer, OutsourcedDataIndex,
					ByteCount);
				if ((ByteCount & 0x1) != 0)
				{
		...
		private int CreateExifBlock(out FlexArray NewExifBlock, int CurrentExifBlockOffset)
		{
			...
			int WriteIndex = 0;
			UpdateIfdPointerTags(out TagItem PrivateDataPointerTag, out TagItem GpsInfoDataPointerTag,
				out TagItem InteroperabilityPointerTag);
			CreateIfdPrimaryData(WriteExifBlock, ref WriteIndex, CurrentExifBlockOffset,
				out int PrivateDataIfdPointerIndex,
				out int GpsInfoDataIfdPointerIndex, out int ThumbnailDataIfdPointerIndex);
			CreateIfdPrivateData(WriteExifBlock, ref WriteIndex, CurrentExifBlockOffset, PrivateDataIfdPointerIndex,
				PrivateDataPointerTag,
				out int InteroperabilityIfdPointerIndex);
			CreateIfdGpsInfoData(WriteExifBlock, ref WriteIndex, CurrentExifBlockOffset, GpsInfoDataIfdPointerIndex,
				GpsInfoDataPointerTag);
			CreateIfdInteroperability(WriteExifBlock, ref WriteIndex, CurrentExifBlockOffset,
				InteroperabilityIfdPointerIndex, InteroperabilityPointerTag);
			if (ImageType == ImageType.Tiff)
			{
				...
				ExifWriteUInt32(WriteExifBlock.Buffer, TiffNextExifBlockPointerIndex,
					0); // Set pointer to next image provisorily to 0
			}
			else
				CreateIfdThumbnailData(WriteExifBlock, ref WriteIndex, CurrentExifBlockOffset,
					ThumbnailDataIfdPointerIndex);
		...
		private void UpdateIfdPointerTags(out TagItem PrivateDataPointerTag, out TagItem GpsInfoDataPointerTag,
			out TagItem InteroperabilityPointerTag)
		{
		...
		private void CreateIfdPrimaryData(FlexArray WriteExifBlock, ref int WriteIndex, int ExifBlockOffset,
			out int PrivateDataIfdPointerIndex,
			out int GpsInfoDataIfdPointerIndex, out int ThumbnailDataIfdPointerIndex)
		{
			Dictionary<ExifTagId, TagItem> PrimaryDataIfdTable = TagTable[(uint)ExifIfd.PrimaryData];
			...
			foreach (TagItem t in PrimaryDataIfdTable.Values)
			{
				if (t.TagId == ExifTagId.ExifIfdPointer)
				{
					...
					PrivateDataIfdPointerIndex =
						WriteIndex + 8; // The "Private Data IFD" pointer is unknown at this time and a dummy
				...
				else if (t.TagId == ExifTagId.GpsInfoIfdPointer)
				{
					...
					GpsInfoDataIfdPointerIndex =
						WriteIndex + 8; // The "GPS Info Data IFD" pointer is unknown at this time and a dummy
					// value is written for it. The index is stored, at which the actual pointer can be written later.
				}
		...
		private void CreateIfdPrivateData(FlexArray WriteExifBlock, ref int WriteIndex, int ExifBlockOffset,
			...
			TagItem PrivateDataPointerTag, out int InteroperabilityIfdPointerIndex)
		{
			...
			if (PrivateDataTagCount > 0)
			{
				...
				ExifWriteUInt32(PrivateDataPointerTag.ValueData, PrivateDataPointerTag.ValueIndex,
					(uint)(ExifBlockOffset + WriteIndex));
				ExifWriteUInt32(WriteExifBlock.Buffer, PrivateDataIfdPointerIndex,
					(uint)(ExifBlockOffset + WriteIndex));
				do
				{
					...
					foreach (TagItem t in PrivateDataIfdTable.Values)
					{
						...
						{
							t.TagType = ExifTagType.ULong;
							InteroperabilityIfdPointerIndex =
								WriteIndex +
								8; // The "Interoperability IFD" pointer is unknown at this time and a dummy
							// value is written for it. The index is stored, at which the actual pointer can be written later.
						}
					...
					ExifWriteUInt32(WriteExifBlock.Buffer, WriteIndex,
						0); // Write null pointer at the end of the tag list
					WriteIndex =
						WriteExifBlock.Length; // Write the next IFD behind the outsourced data of the IFD Private Data
					AddOffsetSchemaTag = CheckIfMakerNoteTagHasMoved(WriteExifBlock.Buffer, MakerNoteDataPointerIndex,
						OffsetSchemaValueIndex, OffsetSchemaTag);
					if (AddOffsetSchemaTag)
					{
						...
						WriteExifBlock.Length =
							iPrivateDataStart; // Free all memory that was allocated for IFD Private Data
		...
		private void CreateIfdGpsInfoData(FlexArray WriteExifBlock, ref int WriteIndex, int ExifBlockOffset,
			int GpsInfoDataIfdPointerIndex, TagItem GpsInfoDataPointerTag)
		{
			Dictionary<ExifTagId, TagItem> GpsInfoDataIfdTable = TagTable[(uint)ExifIfd.GpsInfoData];
			int GpsInfoDataTagCount = GpsInfoDataIfdTable.Count;
			if (GpsInfoDataTagCount > 0)
			{
				...
				WriteIndex =
					WriteExifBlock.Length; // Write the next IFD behind the outsourced data of the IFD GPS Info Data
			}
		}


		private void CreateIfdInteroperability(FlexArray WriteExifBlock, ref int WriteIndex, int ExifBlockOffset,
			...
			TagItem InteroperabilityPointerTag)
		{
			...
			if (InteroperabilityTagCount > 0)
			{
				...
				ExifWriteUInt32(InteroperabilityPointerTag.ValueData, InteroperabilityPointerTag.ValueIndex,
				...
				ExifWriteUInt32(WriteExifBlock.Buffer, WriteIndex, 0); // Write null pointer at the end of the tag list
				WriteIndex =
					WriteExifBlock.Length; // Write the next IFD behind the outsourced data of the IFD Interoperability
			}
		}


		private void CreateIfdThumbnailData(FlexArray WriteExifBlock, ref int WriteIndex, int ExifBlockOffset,
			int ThumbnailDataIfdPointerIndex)
		{
			...
			{
				ExifWriteUInt32(WriteExifBlock.Buffer, ThumbnailDataIfdPointerIndex,
					(uint)(ExifBlockOffset + WriteIndex));
				...
				if (ThumbnailImage != null)
				{
					...
					else
					{
						...
						ExifWriteUInt32(ThumbnailImagePointerTag.ValueData, ThumbnailImagePointerTag.ValueIndex,
							ThumbnailImageOffset);
						...
						Array.Copy(ThumbnailImage, ThumbnailStartIndex, WriteExifBlock.Buffer, ThumbnailImageIndex,
							ThumbnailByteCount);
		...
		private bool CheckIfMakerNoteTagHasMoved(byte[] DataBlock, int MakerNoteDataPointerIndex,
			int OffsetSchemaValueIndex, TagItem OffsetSchemaTag)
		{
			...
			if ((MakerNoteOriginalOffset > 0) && (MakerNoteDataPointerIndex > 0))
			{
				...
				if (MakerNoteOriginalOffset != MakerNoteCurrentOffset)
				{
					...
					if (OffsetSchemaValueIndex > 0)
					{
						...
						ExifWriteUInt32(OffsetSchemaTag.ValueData, OffsetSchemaTag.ValueIndex,
							(uint)NewOffsetDifference);
					}
					else
					{
		...
		private TagItem CreateTagWithReferenceToIfdRawData(byte[] IfdRawData, int IfdRawDataIndex)
		{
			...
			TagItem TempTagItem = new TagItem(TagId, TagType, (int)ValueCount, TagData, ValueIndex, AllocatedByteCount,
				OriginalOffset);
			return (TempTagItem);
		}
		...
		private void InitIfdPrimaryData(byte[] IfdRawData, int IfdRawDataIndex, out int PrivateDataOffset,
			out int GpsInfoDataOffset, out int NextImageOffset)
		{
			PrivateDataOffset = 0;
			GpsInfoDataOffset = 0;
		...
		private void InitIfdThumbnailData(byte[] IfdRawData, int IfdRawDataIndex)
		{
			...
			{
				GetOutsourcedData(ThumbnailImageDataOffset, ThumbnailByteCount, out ThumbnailImage,
					out ThumbnailStartIndex);
			}
		}
		...
		private int EvaluateExifBlock(int PrimaryDataOffset)
		{
			int PrivateDataOffset,
				...
				ThumbnailDataOffset,
				IfdIndex,
				TiffNextExifBlockOffset = 0;
			byte[] IfdTable;

			TagTable = new Dictionary<ExifTagId, TagItem>[ExifIfdCount];
			GetNextIfd(PrimaryDataOffset, out IfdTable, out IfdIndex);
			InitIfdPrimaryData(IfdTable, IfdIndex, out PrivateDataOffset, out GpsInfoDataOffset,
				out ThumbnailDataOffset);
		...
		private bool GetTagValueWithIdCode(ExifTag TagSpec, out string Value, ushort CodePage)
		{
			...
			if (GetTagItem(TagSpec, out t) && (t.TagType == ExifTagType.Undefined) && (t.ValueCount >= IdCodeLength))
			{
				...
				else if (CompareArrays(t.ValueData, t.ValueIndex, IdCodeAscii) ||
				         CompareArrays(t.ValueData, t.ValueIndex, IdCodeDefault))
				{
					IsAsciiCoded = true;
				}
		...
		private bool GetGpsCoordinateHelper(out GeoCoordinate Value, ExifTag ValueTag, ExifTag RefTag, char Cp1,
			char Cp2)
		{
			bool Success = false;
		...
		private static readonly ExifTagId[] InternalTiffTags = new ExifTagId[]
		{
			...
			ExifTagId.Compression, ExifTagId.PhotometricInterpretation, ExifTagId.Threshholding, ExifTagId.CellWidth,
			ExifTagId.CellLength,
			ExifTagId.FillOrder, ExifTagId.StripOffsets, ExifTagId.SamplesPerPixel, ExifTagId.RowsPerStrip,
			ExifTagId.StripByteCounts,
			ExifTagId.MinSampleValue, ExifTagId.MaxSampleValue, ExifTagId.PlanarConfiguration, ExifTagId.FreeOffsets,
			ExifTagId.FreeByteCounts,
			ExifTagId.GrayResponseUnit, ExifTagId.GrayResponseCurve, ExifTagId.T4Options, ExifTagId.T6Options,
			ExifTagId.TransferFunction,
			ExifTagId.Predictor, ExifTagId.WhitePoint, ExifTagId.PrimaryChromaticities, ExifTagId.ColorMap,
			ExifTagId.HalftoneHints,
			ExifTagId.TileWidth, ExifTagId.TileLength, ExifTagId.TileOffsets, ExifTagId.TileByteCounts,
			ExifTagId.ExtraSamples,
			ExifTagId.SampleFormat, ExifTagId.SMinSampleValue, ExifTagId.SMaxSampleValue, ExifTagId.TransferRange,
			ExifTagId.YCbCrCoefficients, ExifTagId.YCbCrSubSampling, ExifTagId.YCbCrPositioning,
			ExifTagId.ReferenceBlackWhite,
			(ExifTagId)0x0200, (ExifTagId)0x0201, (ExifTagId)0x0202, (ExifTagId)0x0203, (ExifTagId)0x0205,
			(ExifTagId)0x0206,
		...
		{
			TagItem NewTag = new TagItem(TagItemToBeCopied.TagId, TagItemToBeCopied.TagType,
				TagItemToBeCopied.ValueCount);
			Array.Copy(TagItemToBeCopied.ValueData, TagItemToBeCopied.ValueIndex, NewTag.ValueData, 0,
				NewTag.ByteCount);
			if (SwapByteOrder)
			{
		...
		private class TagItem
		{
			...
			public int
				ValueCount; // Number of values of the tag. In general this is not the number of bytes of the tag!
			...
			public int
				ByteCount // The number of bytes required to store the tag data, i. e. the number of bytes actually used for the tag data.
			{
				get { return (ExifData.GetTagByteCount(TagType, ValueCount)); }
			}

			public int
				OriginalDataOffset; // Offset of outsourced data in the image file. 0 = Tag does not have outsourced data or
	...
	public enum ExifTag
	{
		...
		JpegInterchangeFormatLength =
			(ExifIfd.ThumbnailData << ExifData.IfdShift) | ExifTagId.JpegInterchangeFormatLength,
	}
	...
	public enum StrCodingFormat
	{
		...
		TypeUndefinedWithIdCode =
			0x00030000 // Tag type is "ExifTagType.Undefined" and an additional ID code is present. A null terminating character is not present.
	};
	...
	public enum StrCoding
	{
		...
		UsAscii_Undef =
			StrCodingFormat.TypeUndefined | 20127, // For the tags "ExifVersion", "FlashPixVersion" and others.

		Utf16Le_Byte =
			StrCodingFormat.TypeByte |
			1200, // For the Microsoft tags "XpTitle", "XpComment", "XpAuthor", "XpKeywords" and "XpSubject".
		...
		public override string Message
		{
			get
			{
				...
				switch (ErrorCode)
				{
					...
					case ExifErrCode.ExifDataAreTooLarge:
						s = "EXIF data are too large: 64 kB for JPEG files, 2 GB for TIFF files!"; break;
